...
│   └── converter/       # PO 转换器（如果其他层也需要converter一样的在对应层的位置定义converter包）
    └── persistence/    # 持久化实现
...
服务层: cfsvr → monitor
工作流层: workflow → session → agent → tool → model
资源层: resource (独立)
```
...
- 统一返回 Result<T> 类型
- 所有Controller和在Controller里出现的请求或返回的实体或实体上在api里要使用到的字段都需要带上swagger的注解来标识这个接口/类/类字段的功能
