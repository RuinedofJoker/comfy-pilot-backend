...
**说明**：
- 继承 `BasePO` 后自动拥有：`id`, `createTime`, `updateTime`, `createBy`, `updateBy`, `isDeleted`
- 这些字段会自动填充，无需手动设置
...
// createTime, updateTime, createBy, updateBy 会自动填充

// 更新数据
po.setName("修改后的名称");
workflowMapper.updateById(po);
// updateTime, updateBy 会自动更新
```
...
```json
{
  ...
  "data": {
    "id": 1,
    "name": "测试工作流",
    "createTime": "2026-01-14T19:30:45"
  },
