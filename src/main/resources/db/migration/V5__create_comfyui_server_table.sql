...
COMMENT ON COLUMN comfyui_server.auth_mode IS '认证模式（NULL/BASIC_AUTH/OAUTH2等）';
...
COMMENT ON COLUMN comfyui_server.source_type IS '注册来源（MANUAL-手动创建，CODE_BASED-代码注册）';
...
-- 2. 创建索引
CREATE INDEX IF NOT EXISTS idx_comfyui_server_source_type ON comfyui_server(source_type);
