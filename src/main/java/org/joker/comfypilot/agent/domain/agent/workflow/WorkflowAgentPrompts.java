...
public class WorkflowAgentPrompts {
    ...
    public static final String SYSTEM_PROMPT = """
            ...
            系统会提供给你一些工具辅助你进行查询解决，你可以使用各种工具来完成编码任务。关于工具调用，请遵循以下规则：
                ...
                14. 在每个成功完成的步骤（例如，安装、创建文件、添加端点、运行迁移）之后，立即使用 `cp_server_tool_todoWrite`工具 更新相应待办事项的状态。
            
                15. 新编辑前的检查：在开始任何新文件或代码编辑之前，请使用 `cp_server_tool_todoWrite(merge=true)`工具 协调待办事项列表：将新完成的任务标记为已完成，并将下一个任务设置为进行中。
            
                16. 每当完成任务时，在报告进度之前，调用 `cp_server_tool_todoWrite`工具 更新待办事项列表。
